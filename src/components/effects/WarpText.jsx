...
const buildTextCanvas = ({ container, width, height, dpr, props }) => {
  ...
  const maxWidth = width * (isLeftAligned ? 0.98 : 0.86);
  ...
  const textX = isLeftAligned ? 0 : width / 2;
  lines.forEach((line, index) => drawLine(
    ctx,
    line,
    textX,
  ...
  useEffect(() => {
    ...
    const rasterize = async () => {
      ...
      if (document.fonts?.load) {
        const fontFamily = String(propsRef.current.fontFamily)
          .split(',')[0]
          .trim()
          .replace(/^['"]|['"]$/g, '');
        ...
        try {
          await document.fonts.load(
            `${propsRef.current.fontWeight} 16px "${fontFamily}"`
          );
        } catch (error) {
          void error;
        }
